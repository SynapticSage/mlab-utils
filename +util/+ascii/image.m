function asciimg = image(img,axelWidth,outWidth,mustRescale,gamma,filename)
...
if nargin < 3
    outWidth = ceil(size(img,2)/axelWidth);
end
% Final size
wa = outWidth;
...
for y=1:ha
    for x=1:wa
        ...
        if pos>size(palettes{i}.grays)
            pos = size(palettes{i}.grays);
        end
        if pos<1
            pos=1;
        end
