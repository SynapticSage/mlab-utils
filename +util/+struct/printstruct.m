...
for i = 1:length(S)
    cS = S{i};
    if length(cS)==1
        str = recFieldPrint(cS, nindent, nlevels, printcontents, maxarraylength, structname);
        str = [cellstr(structname); str];
    else
        str = [];
        for i = 1:length(cS)
            tmpstr = recFieldPrint(cS(i), nindent, nlevels, printcontents, maxarraylength, structname);
...
for iField = 1 : length(vectorFields)
    ...
    if length(Structure.(Field)) > maxArrayLength
        varStr = createArraySize(Structure.(Field), ['Array' '(' class(Structure.(Field)) ')']);
    else
        varStr = sprintf('%g ', Structure.(Field));
